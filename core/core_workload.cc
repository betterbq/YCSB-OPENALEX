...
const string CoreWorkload::BATCHREAD_PROPORTION_PROPERTY =
    "batchreadproportion";
const string CoreWorkload::BATCHREAD_PROPORTION_DEFAULT = "0.0";

const string CoreWorkload::BATCHREADNUM_PROPERTY =
    "batchreadnum";
const string CoreWorkload::BATCHREADNUM_DEFAULT = "40";

const string CoreWorkload::READTOTALNUMFORBATCH_PROPERTY =
    "readtotalnumforbatch";
const string CoreWorkload::READTOTALNUMFORBATCH_DEFAULT = "0";
...
void CoreWorkload::Init(const utils::Properties &p) {
  ...
  double batchread_proportion = std::stod(p.GetProperty(
     BATCHREAD_PROPORTION_PROPERTY, BATCHREAD_PROPORTION_DEFAULT));

  batch_read_num_ = std::stoi(p.GetProperty(BATCHREADNUM_PROPERTY,
                                         BATCHREADNUM_DEFAULT));
  read_total_num_for_batch_ = std::stoi(p.GetProperty(READTOTALNUMFORBATCH_PROPERTY,
		  READTOTALNUMFORBATCH_DEFAULT));
  ...
  if (batchread_proportion > 0) {
    op_chooser_.AddValue(BATCHREAD, batchread_proportion);
  }
