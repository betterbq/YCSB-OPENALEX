...
class CoreWorkload {
 public:
  ...
  static const std::string BATCHREAD_PROPORTION_PROPERTY;
  static const std::string BATCHREAD_PROPORTION_DEFAULT;

  static const std::string BATCHREADNUM_PROPERTY;
  static const std::string BATCHREADNUM_DEFAULT;

  static const std::string READTOTALNUMFORBATCH_PROPERTY;
  static const std::string READTOTALNUMFORBATCH_DEFAULT;
  /// 
  ...
  int read_total_num_for_batch_;
  int done_read_num_for_batch_;
  ///
  ...
  virtual void NextTransactionScanKey(std::string &start_key,std::string&end_key,int len);
  ...
  int batch_read_num() const {return batch_read_num_;}
  double BatchReadWeight();
  CoreWorkload() :
      field_count_(0), batch_read_num_(0),read_total_num_for_batch_(0),
	  done_read_num_for_batch_(0),read_all_fields_(false), write_all_fields_(false),
...
inline void CoreWorkload::NextTransactionScanKey(std::string &start_key,std::string &end_key,int len){
	uint64_t key_num;
	do{
		key_num=key_chooser_->Next();
	}while(key_num>insert_key_sequence_.Last());
	...
	end_key=start_key;
	int index=log10(record_count_*10000/len);
	end_key[index]++;
}
...
inline double CoreWorkload::BatchReadWeight() {
  return op_chooser_.ValueWeight(BATCHREAD);
}
} // ycsbc
